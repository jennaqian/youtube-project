...
export default class Youtube extends Component {
    ...
    getVideos = async () => {
        console.log(process.env.REACT_APP_keyAPI)
        const {data} = await axios.get(`https://youtube.googleapis.com/youtube/v3/search?videoId&key=${process.env.REACT_APP_keyAPI }`)
        this.setState ({
            videos : data.items[0]
        })
        ...
        return (
            <div>
                ...
                <form onSubmit={this.handleSubmit}>
                    ...
                    <input type="text" onChange={this.handleChange} placeholder="Search here..." id='input' value={this.state.input}/>
            ...
            <main>
                ...
                {this.state.show && this.state.videos.map((video, i)=> {
                    return (
